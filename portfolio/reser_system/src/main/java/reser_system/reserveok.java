	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(ckmsg.equals("fail")) {
				...
				if(msg.equals("fail")) {
				pr.write("<script>alert('펜션 예약시스템 오류입니다.'); location.href='http://192.168.100.210:8080/reser_system/reserve.html';</script>");	
				}
				else {								
				if(msg2.equals("fail")) {				
					pr.write("<script>alert('펜션 예약시스템 오류입니다.'); location.href='http://192.168.100.210:8080/reser_system/reserve.html';</script>");	
					...
					if(msg3.equals("fail")) {
						pr.write("<script>alert('예약 번호 발송 오류입니다.'); location.href='http://192.168.100.210:8080/reser_system/reserve.html';</script>");									
						}
						else {
						pr.write("<script>"
